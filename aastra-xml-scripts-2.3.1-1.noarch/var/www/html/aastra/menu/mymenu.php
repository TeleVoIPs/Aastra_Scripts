	...
	case 'listr':
		...
		if ((empty($menu_source)) || (!file_exists($menu_source.'.menu'))) 
			...
			if(count($config['menu'])>0)
				...
				if(Aastra_is_icon_menu_supported())
					{
					require_once('AastraIPPhoneIconMenu.class.php');
					$object=new AastraIPPhoneIconMenu();
					$object->setDestroyOnExit();
					$object->setTopTitle('Applications','',1);
					$object->addIcon('1','http://'.$AA_XML_SERVER.'/'.$AA_XMLDIRECTORY.'/icons/aastra.png');
					...
					foreach($config['menu'] as $key=>$value)
						...
						$icon=preg_replace($search, $replace, $all[$value]['icon']);
						if(strstr($uri,'rss')) $counter=rand(0,15);
						else $counter=0;
						if($icon!='') 
							{
							$object->addIcon($icon_index, $icon);
							$object->addEntry($index,$icon_index,$all[$value]['title'],$uri.'','',$counter);
							$icon_index++;
							}
						else $object->addEntry($index,'1',$all[$value]['title'],$uri,'','',$counter);
						$index++;
						}
					}
				else
