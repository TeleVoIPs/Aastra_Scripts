...
if($user!='')
	...
	$park=Aastra_get_park_config_Asterisk($user);
...
if(($action=='register') and Aastra_is_ledcontrol_supported())
    ...
    if(isset($park['parkpos'])) {
			$parked=Aastra_get_parked_calls_Asterisk();
			$status = 'off';
	    foreach($parked as $key => $value) {
	      if($value[0] >= $park['parkmin'] && $value[0] <= $park['parkmax']) {
	        $status = 'slowflash';
	      }
	    }
		}
